...
{
    private const string ConfigPath = "res://__MOD_ID__/config/card_replacements.json";
    ...
    public static void EnsureLoaded()
    {
        ...
        try
        {
            ...
            {
                GD.PushWarning($"[__MOD_ID__] Replacement config missing or empty: {ConfigPath}");
                return;
            }
            ...
            GD.Print($"[__MOD_ID__] Loaded {Entries.Count} replacement config entr{(Entries.Count == 1 ? "y" : "ies")}.");
        }
        catch (Exception ex)
        {
            GD.PushWarning($"[__MOD_ID__] Failed to load replacement config: {ex.Message}");
            Entries.Clear();
        }
    }
